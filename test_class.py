...
class FinGPTLLM(BaseLLM):
    """FinGPT implementation using Llama-3-8B with LoRA adapter for finance"""
    ...
    def __init__(self, config: ModelConfig):
        ...
        self._model = None
        self._device = None
        self.base_model = "meta-llama/Meta-Llama-3-8B"
        self.peft_model = "./fingpt-mt_llama3-8b_lora"
    
    def _load_model(self):
        """Lazy load the FinGPT model on first use"""
        if self._model is not None:
            return
        ...
        try:
            ...
            device = "cuda" if torch.cuda.is_available() else "cpu"
            print(f"Loading FinGPT model from {self.base_model} with LoRA adapter {self.peft_model} on {device}...")
            
            self._tokenizer = LlamaTokenizerFast.from_pretrained(self.base_model, trust_remote_code=True)
            ...
            self._model = PeftModel.from_pretrained(self._model, self.peft_model)
            self._model = self._model.eval()
            self._model = self._model.to(device)
            self._device = device
            print(f"FinGPT model loaded successfully!")
    ...
    def generate(self, prompt: str, context: Dict[str, Any]) -> str:
        try:
            ...
            # Build the prompt in FinGPT format
            system_prompt = self._build_system_prompt(context)
            full_prompt = f"{system_prompt}\n\nInput: {prompt}\nAnswer:"
            
            # Tokenize and generate
            import torch
            inputs = self._tokenizer([full_prompt], return_tensors='pt', padding=True, max_length=512).to(self._device)
    ...
    def _build_fingpt_prompt(self, context: Dict[str, Any]) -> str:
        """Build prompt in FinGPT instruction format"""
        persona = context.get("persona", {})
        
        # Build instruction prompt for FinGPT (finance sentiment/topic analysis)
        instruction = "You are a personalized financial advisor assistant."
        
        prompt = f'''Instruction: You are a personalized financial advisor assistant. Provide helpful, personalized financial advice based on the client's profile and history. '''
...
def create_llm(config: ModelConfig) -> BaseLLM:
    ...
    elif provider == "fingpt":
        return FinGPTLLM(config)
    else:
...
def main():
    ...
    if args.enable_llm_eval:
        ...
        eval_base_url = args.eval_base_url or os.environ.get("OPENAI_BASE_URL", "")
    ...
    model_configs = {
        ...
        "llama-3.1-8b": ModelConfig(
            ...
            base_url="./Llama-3.1-8B"
        ),
        "fingpt": ModelConfig(
            name="FinGPT",
            provider="fingpt",
            model_id="fingpt-mt_llama3-8b_lora",
            base_url=None
        )
    }
    ...
    for model_key in args.models:
        ...
        if not config.api_key and config.provider not in ["gpu", "local", "fingpt"]:
